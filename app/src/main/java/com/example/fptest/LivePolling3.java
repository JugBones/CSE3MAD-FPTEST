...
public class LivePolling3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_live_polling3);

        candidate1TextView = findViewById(R.id.dpd1TextView);
        candidate2TextView = findViewById(R.id.dpd2TextView);
        candidate3TextView = findViewById(R.id.dpd3TextView);
        candidate4TextView = findViewById(R.id.dpd4TextView);
        candidate5TextView = findViewById(R.id.dpd5TextView);
        ImageButton home_Btn = findViewById(R.id.home_btn);
        pieChart = findViewById(R.id.pieChart);
        barChart = findViewById(R.id.barChart);
        Spinner chartTypeSpinner = findViewById(R.id.chartTypeSpinner);

        home_Btn.setOnClickListener(v -> startActivity(new Intent(LivePolling3.this, HomePage.class)));

        DatabaseReference candidatesRef = FirebaseDatabase.getInstance().getReference("candidates/DpdCandidate");

        candidatesRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                Integer candidate1Votes = snapshot.child("dpd1/votes").getValue(Integer.class);
                Integer candidate2Votes = snapshot.child("dpd2/votes").getValue(Integer.class);
                Integer candidate3Votes = snapshot.child("dpd3/votes").getValue(Integer.class);
                Integer candidate4Votes = snapshot.child("dpd4/votes").getValue(Integer.class);
                Integer candidate5Votes = snapshot.child("dpd5/votes").getValue(Integer.class);
